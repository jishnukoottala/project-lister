...
export default function ProjectListing() {
  ...
  const { loading, error, projects, page,  fetchNext, fetchPrevious } = useFetchProjects()
  ...
  return (
    <div className="mt-5 container mx-auto px-4 flex flex-col justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-7 gap-x-1 gap-y-2">
         {projects?.length > 0 && !loading &&  projects.map((project)=> (<ProjectItem key={project.id} {...project} />))} </div>
         {projects?.length > 0 && !loading && (<Pagination active={true} 
         showPrevious={page > 1}
         fetchNext={fetchNext} fetchPrevious={fetchPrevious} showNext={!(projects?.length < 5)} />)}
