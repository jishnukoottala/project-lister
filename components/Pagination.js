...
export default function Pagination({ active, fetchNext, fetchPrevious, showNext, showPrevious }) {

const defaultClassName = `border border-zinc-200
...
const activeClassName = `border border-sky-200 text-sky-500 text-base hover:text-cyan-900 hover:border-sky-800`;


  return (
    <div className="mt-4 flex justify-end">
      <ul className="flex items-center -mx-[6px]">
      {showPrevious && (<li className="px-[6px]" onClick={fetchPrevious}>
         <span className={`
            w-9
            h-9
            flex
      ...
      {showNext && (<li className="px-[6px]" onClick={fetchNext}>
         <span className={`
            w-9
            h-9
            flex
